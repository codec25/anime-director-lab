...
function adcr_sequence_review_warning(array $shot,string $media):array{
  if(empty($shot['sequence_plan_id'])||$media==='')return[];
  $beat=max(1,(int)($shot['sequence_beat']??1));$required=['identity','wardrobe','location','lighting','palette'];if($beat>1)$required[]='screen_direction';$checks=adcr_review($shot)['checks'];$unchecked=[];$failed=[];foreach($required as$k){$v=(string)($checks[$k]??'unchecked');if($v==='fail')$failed[]=$k;elseif(!in_array($v,['pass','warn'],true))$unchecked[]=$k;}
  if($failed)return[['code'=>'VISUAL_CONTINUITY_FAIL','label'=>'Continuity needs correction: '.implode(', ',array_map(static fn($x)=>str_replace('_',' ',$x),$failed))]];
  if($unchecked)return[['code'=>'VISUAL_REVIEW_REQUIRED','label'=>'Quick visual continuity check required before this sequence take can be kept']];
  return[];
}
function adcr_metadata_warnings(array $state,array $scene,array $shot,string $dominantRatio):array{
  ...
  if(!empty($shot['sequence_plan_id'])){$contract=is_array($shot['continuity_contract']??null)?$shot['continuity_contract']:[];$expected=array_values(array_filter(array_map('strval',(array)($contract['cast_ids']??[]))));$actual=array_values(array_filter(array_map('strval',(array)($shot['character_version_ids']??[]))));$missing=array_values(array_diff($expected,$actual));if($missing)$w[]=['code'=>'CAST_BINDING_DRIFT','label'=>'This planned beat lost '.count($missing).' locked cast binding'.(count($missing)===1?'':'s')];$sourceId=trim((string)($shot['continuity_from_shot_id']??''));if($sourceId!==''){$source=adcr_find((array)$state['shots'],$sourceId);if(!$source||adcr_take_url($state,$source)==='')$w[]=['code'=>'SEQUENCE_SOURCE_MISSING','label'=>'The previous approved moving shot is not available as a continuity source'];}}
  foreach(adcr_sequence_review_warning($shot,$media)as$x)$w[]=$x;
...
function adcr_payload(array $state):array{
  $scenes=[];foreach((array)$state['scenes']as$scene){if(!is_array($scene))continue;$sceneShots=[];foreach((array)($scene['shot_ids']??[])as$id){$shot=adcr_find((array)$state['shots'],(string)$id);if($shot)$sceneShots[]=$shot;}if(!$sceneShots){foreach((array)$state['shots']as$shot)if(is_array($shot)&&(string)($shot['scene_id']??'')===(string)($scene['id']??''))$sceneShots[]=$shot;}
    ...
    foreach($sceneShots as$shot){$lines=array_values(array_filter((array)($shot['dialogue']??[]),'is_array'));$speakers=array_values(array_unique(array_filter(array_map(static fn($l)=>(string)($l['speaker_name']??''),$lines))));$rows[]=['id'=>(string)$shot['id'],'number'=>(int)($shot['number']??$shot['shot_number']??0),'title'=>(string)($shot['title']??'Shot'),'intent'=>(string)($shot['intent']??$shot['direction']??''),'ratio'=>(string)($shot['ratio']??''),'status'=>(string)($shot['status']??'draft'),'camera_direction'=>(string)($shot['camera_direction']??''),'media_url'=>adcr_take_url($state,$shot),'review'=>adcr_review($shot),'warnings'=>adcr_metadata_warnings($state,$scene,$shot,$dominant),'dialogue_count'=>count($lines),'speakers'=>$speakers,'world_version_id'=>$shot['world_version_id']??null,'selected_take_id'=>$shot['selected_take_id']??null,'sequence_plan_id'=>$shot['sequence_plan_id']??null,'sequence_beat'=>(int)($shot['sequence_beat']??0),'continuity_delta'=>(string)($shot['continuity_delta']??''),'continuity_contract'=>is_array($shot['continuity_contract']??null)?$shot['continuity_contract']:[]];}

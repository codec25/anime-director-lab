...
function adcr_take_url(array $state,array $shot):string{
  ...
  if(!$take){$matches=array_values(array_filter((array)$state['takes'],static fn($t):bool=>is_array($t)&&(string)($t['shot_id']??'')===(string)($shot['id']??'')));$take=$matches?$matches[count($matches)-1]:null;}
...
function adcr_metadata_warnings(array $state,array $scene,array $shot,string $dominantRatio):array{
  ...
  $lines=array_values(array_filter((array)($shot['dialogue']??[]),'is_array'));if($lines){$missing=0;foreach($lines as$l)if(!is_array($l['audio_reference']??null))$missing++;if($missing)$w[]=['code'=>'DIALOGUE_AUDIO_MISSING','label'=>$missing.' dialogue '.($missing===1?'line is':'lines are').' not voiced yet'];}
  $passes=array_values(array_filter((array)($shot['precision_passes']??[]),'is_array'));if($passes){$pending=0;foreach($passes as$p)if((string)($p['status']??'')!=='completed')$pending++;if($pending)$w[]=['code'=>'PRECISION_PENDING','label'=>$pending.' precision '.($pending===1?'pass is':'passes are').' incomplete'];}
  return$w;
}

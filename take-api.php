...
function adtake_gate(array $shot):array{
  if(empty($shot['sequence_plan_id']))return['required'=>false,'ready'=>true,'required_checks'=>[],'blockers'=>[],'unchecked'=>[]];
  $beat=max(1,(int)($shot['sequence_beat']??1));
  $required=['identity','wardrobe','location','lighting','palette'];
  if($beat>1)$required[]='screen_direction';
  $review=is_array($shot['continuity_review']??null)?$shot['continuity_review']:[];
  $checks=is_array($review['checks']??null)?$review['checks']:[];
  $unchecked=[];$blockers=[];
  foreach($required as$k){$v=(string)($checks[$k]??'unchecked');if($v==='fail')$blockers[]=$k;elseif(!in_array($v,['pass','warn'],true))$unchecked[]=$k;}
  return['required'=>true,'ready'=>!$blockers&&!$unchecked,'required_checks'=>$required,'blockers'=>$blockers,'unchecked'=>$unchecked,'reviewed_at'=>$review['reviewed_at']??null];
}

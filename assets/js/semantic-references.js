...
function boot(){load().catch(e=>toast(e.message));document.addEventListener('ad:shots-rendered',patchShots);document.addEventListener('ad:ui-updated',patchWorld);document.addEventListener('click',e=>{if(e.target.closest('[data-reference-shot]')||e.target.closest('#worldReference'))setTimeout(()=>load().catch(()=>{}),1200)})}

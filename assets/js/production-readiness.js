...
function boot(){ensure();refresh();document.addEventListener('ad:state-changed',schedule);document.addEventListener('ad:take-selected',schedule);const root=$('main')||document.body;new MutationObserver(()=>{if(!$('#productionReadiness'))ensure()}).observe(root,{childList:true,subtree:true})}

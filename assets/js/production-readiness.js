...
function boot(){ensure();refresh();document.addEventListener('ad:state-changed',schedule);document.addEventListener('ad:take-selected',schedule);document.addEventListener('click',e=>{if(e.target.closest('[data-stage]'))schedule()},true);const root=$('main')||document.body;new MutationObserver(()=>{if(!$('#productionReadiness'))ensure()}).observe(root,{childList:true,subtree:true})}
